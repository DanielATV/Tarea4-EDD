...
int main(){
    ...
    for (i = 0 ; i < consultas ; i++){
        ...
        for (z=first(G,menor);z.ciudad != nVecinos(G,menor) ; z = nextg(G,menor)){
            ...
            if((getMark(G,z.ciudad) != i) && ((ciudad_cumple > z.ciudad && nVecinos(G,z.ciudad) >= num_amigos)){
                cont = 0;
                for(w = first(G,z.ciudad); w.ciudad != nVecinos(G,z.ciudad); w =nextg(G,z.ciudad))
                  if(getMark(G,w.ciudad) == i) cont ++;
                if (cont == num_amigos)
                  ciudad_cumple = z.ciudad;
            }
        }
