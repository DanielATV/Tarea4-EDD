...
int main(){
    ...
    for (i=0;i<consultas;i++){
        ...
        for (z=first(G,ciudades[0]);z != NULL;z = nextg(z)){
            ...
            for(k=1;k<num_amigos;k++){
                if (isEdge(G,z->info.ciudad,ciudades[k]) == -1)break;

                else{
                    if ((z->info.ciudad < ciudad_cumple) && (getMark(G,z->info.ciudad) !=-1)) ciudad_cumple = z->info.ciudad;
                }
